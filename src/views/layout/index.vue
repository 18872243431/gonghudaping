...
const allmenus = computed(() => {
  return store.getters.menus.filter((menu) => menu.path !== "/home");
});
...
function logout() {
  // const loginPath = Cookies.get("loginType") || "login";
  // window.location.href = `/${loginPath}.html`;
...
/* 下拉菜单字体统一调整（作用于弹出层） */
:deep(.menu-popover .el-menu-item) {
  font-family: var(--font-family-title);
  font-size: 22px;
}
</style>
