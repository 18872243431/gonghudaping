...
import {
  ...
  getSecurityOverview,
  getCountryIpTop3,
  getRealTimeDataSecurityEvents,
  getDataSecurityEvents,
  getExposedDataAssets,
  getRiskEnterpriseRanking,
  getDataFlowDynamic,
  getIndustrialInternetMonitoring,
  getIndustrialInternetMonitoringDetail,
  getCountryIpRank,
  getDarkWebMonitor,
} from "@/api/datasecurity";
import { getAllMockData } from "@/api/allMockJson";
import mapRes from "../data.json";
...
export function useLeft() {
  ...
  async function getIndustrialInternetMonitoringDetailData() {
    const data = {
      timeType: activeTab.value,
    };
    const res = await getIndustrialInternetMonitoringDetail(data);
    if (res.code == 0) {
  ...
  async function getIndustrialInternetMonitoringData() {
    const data = {
      timeType: activeTab.value,
    };
    const res = await getIndustrialInternetMonitoring(data);
    if (res.code === 0) {
...
export function useCenter() {
  ...
  async function getSecurityOverviewData() {
    ...
    const res = await getSecurityOverview(data);
    if (res.code == 0) {
  ...
  async function getCountryIpTop3Data(country) {
    const data = {
      timeType: activeTab.value,
      dstIpCountry: country,
    };
    const res = await getCountryIpTop3(data);
    if (res.code == 0) {
      // console.log(res.data);
  ...
  async function getRealTimeDataSecurityEventsData() {
    const data = {
      timeType: activeTab.value,
    };
    const res = await getRealTimeDataSecurityEvents(data);
  ...
  async function getDataFlowDynamicData() {
    const data = {
      timeType: activeTab.value,
    };
    const res = await getDataFlowDynamic(data);
    // const res = mapRes;
  ...
  async function getCountryIpRankData() {
    const data = {
      timeType: activeTab.value,
    };
    const res = await getCountryIpRank(data);
    if (res.code == 0) {
      attackData.value[0].list = res.data
        .filter((item) => item.key !== "中国")
        .map((item) => {
          return {
            label: item.key,
            value: item.doc_count,
...
export function useRight() {
  ...
  async function getDataSecurityEventsData() {
    const data = {
      timeType: activeTab.value,
    };
    const res = await getDataSecurityEvents(data);
    if (res.code == 0) {
  ...
  async function getExposedDataAssetsData() {
    const data = {
      timeType: activeTab.value,
    };
    const res = await getExposedDataAssets(data);
    if (res.code == 0) {
  ...
  async function getRiskEnterpriseRankingData() {
    const data = {
      timeType: activeTab.value,
    };
    const res = await getRiskEnterpriseRanking(data);
    if (res.code == 0) {
      // console.log(res.data);
      // companyList.value.forEach((item, index) => {
      //   item.name = res.data[index].corpName;
      //   item.value1 = res.data[index].attackedTotal;
      //   item.value2 = res.data[index].ipTotal;
      //   item.tags = res.data[index].categoryCnList;
      // });
  ...
  // 获取暗网数据泄露与售卖监测数据
  async function getDarkWebMonitorData() {
    const data = {
      timeType: activeTab.value,
      wdId:'datasecurity'
    };
