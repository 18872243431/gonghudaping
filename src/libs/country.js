...
/**
 * [getNationalPng 获取图片名字]
 * @cname  type 国家的cname
 * @return brief 图片的name
 */
export function getNationalPng(cname) {
  if (!cname) return "";
...
/**
 * [getCountryImg 获取所有国家对应的图片]
 */
export function getCountryImg() {
  let json = {};
  data.forEach((item) => {
    let name = item.brief.toLowerCase();
    let img = require(`public/countryFlag/${name}.png`);
    json[name] = {
      align: "center",
      width: 25,
      height: 20,
...
export async function init(data) {
  for (let i = 0; i < data.length; i++) {
    ...
    if (item.nationalName) {
      let img = "";
      await import(`public/countryFlag/${item.nationalName}.png`).then(
        (result) => {
          img = result.default;
          item["national"] = {
