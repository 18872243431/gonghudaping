...
function fixed(num, count = 3) {
  return num.toString().indexOf(".") > -1 ? num.toFixed(count) : num;
}
// 格式化数字
export function formatNumber(value) {
  if (typeof value === "number") {
    ...
    if (value < 1) {
      ...
      value = fixed(value / 10000, 3);
      unit = "万";
    } else if (value >= 100000000) {
      value = fixed(value / 100000000, 3);
      unit = "亿";
    }
    return {
      value,
...
// 获取数组的最大值
export function getMaxValueByKey(list, key) {
  if (!Array.isArray(list) || list.length === 0) return 0;
  const temp = key ? list.map((item) => item[key]) : list;
...
// 映射视觉最大值
export function getVisualMax(num) {
  if (num < 1000) {
    return 1000;
  }
  if (num < 10000) {
  ...
  return Number(`${Number(fixed(num / 10000, 0)) + 1}0000`);
}

// 设置地图散点图大小
export function setSize(val, min = 10, max = 40, flag, baseNum = 250) {
  let k = baseNum / (max - min);
...
//计算数量级
function getNextMagnitude(maxValue) {
  // 计算最大值的对数
  const logValue = Math.log10(maxValue);

  // 确定最近的量级
  const magnitude = Math.ceil(logValue);

  // 计算最近的量级值
  const nextMagnitudeValue = Math.pow(10, magnitude) / 10;

  return nextMagnitudeValue;
}

//向上取值
export function roundUp(value) {
  const max =
    Math.ceil(value / getNextMagnitude(value)) * getNextMagnitude(value);
  return max;
}
