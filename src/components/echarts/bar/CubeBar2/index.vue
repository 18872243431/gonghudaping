...
const props = defineProps({
  ...
  cubeOption: {
    type: Object,
    default: () => {
      return { width: 24, height: 6, space: 6 };
    },
  },
  single: {
    type: Number,
...
const option = computed(() => {
  ...
  let series = [
    {
      ...
      data: props.data[props.data.length - 2].list.map((item) => item.value),
    },
  ];
  if (props.lineShow) {
    series.push({
  ...
  return {
    ...
    yAxis: [{ ...yAxis, ...props.option?.yAxis[0] }, { ...yAxis, ...props.option?.yAxis[1] }],
