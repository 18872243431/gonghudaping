<template>
  <div class="chart-view" ref="view">
    <!-- :style="`height:${data.length <= 10 ? '240%' : '400%'};`" -->
    <v-chart
      :style="`height:${
        data.length <= 10 ? '240%' : `${Math.ceil((data.length / 5) * 120)}%`
      };`"
      class="chart"
      ref="barChart"
      :option="option"
...
const labelList = computed(() => {
  return props.data.map((item, index) => `${item.label}`).reverse();
});
...
const option = computed(() => {
  return {
    ...
    grid: {
      left: props.option?.grid.left || 20,
      bottom: 20,
      top: 20,
...
function thousands(num) {
  if (num < 10000000) {
    ...
    let showNum = (num / 10000000).toFixed(2) + "千万";
    return showNum;
  } else {
    let showNum = (num / 100000000).toFixed(2) + "亿";
    return showNum;
  }
}
</script>
