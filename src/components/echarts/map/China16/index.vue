  ...
  return {
    ...
    tooltip: {
      ...
      formatter: (params) => {
        ...
        const mvItem = (props.data?.map || []).find((d) => d.name === name);
        const cvItem = (props.data?.cube || []).find((d) => d.name === name);
        const mv = mvItem ? mvItem.value : undefined;
        const cv = cvItem ? cvItem.value : undefined;
        const lines = [];
        if (mv !== undefined) lines.push(`${props.name1 || "被攻击次数"}：${mv}`);
        if (cv !== undefined) lines.push(`${props.name2 || "风险企业"}：${cv}`);
